...
.PHONY: manifests
manifests: controller-gen ## Generate WebhookConfiguration and ClusterRole objects. CRDs are managed in vitistack/crds.
...
KIND_CLUSTER ?= proxmox-operator-test-e2e
...
SBOM_PROJECT_NAME ?= proxmox-operator
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name proxmox-operator-builder
	$(CONTAINER_TOOL) buildx use proxmox-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm proxmox-operator-builder
	rm Dockerfile.cross
